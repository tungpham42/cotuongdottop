...
@section('aboveBoard')
<h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="赤で遊ぶ">あなたはホストです</h5>
...
<div id="copy-url-black" class="input-group my-1 w-50 mx-auto pulse-light" data-toggle="tooltip" data-placement="bottom" data-original-title="クリックしてコピー">
  <div class="input-group-prepend">
    <span class="input-group-text" id="url-addon-black"><i class="fal fa-copy"></i></span>
  </div>
  <input data-step="1" data-intro="リンクをコピーして友達を招待して一緒に遊ぶにはここをクリックしてください" type="text" class="form-control" id="url-black" value="{{ url('/') }}/rumu/{{ $roomCode }}/geesuto">
</div>
<p id="room-code" class="w-100 text-center mt-0 mb-1">
  <span data-step="4" data-intro="このルームコードを使ってマッチを検索してください" class="alert alert-dark d-inline-block" role="alert" data-toggle="tooltip" data-placement="bottom" data-original-title="このルームコードをコピー"><i class="fad fa-trophy-alt"></i> ルームコード: <strong style="cursor: pointer;">{{ $roomCode }}</strong></span>
  <input type="hidden" id="room-code-input" value="{{ $roomCode }}">
</p>
<div class="text-center w-100 my-1">
{!! QrCode::backgroundColor(240, 240, 240)->color(240, 65, 36)->size(300)->generate(url('/').'/rumu/'.$roomCode.'/shotai-sareta'); !!}
</div>
@if ($room['pass'] != null)
<div data-step="2" data-intro="部屋のパスワードを変更するにはここをクリックしてください" id="change-pass" class="input-group my-1 w-50 mx-auto">
  <label class="m-auto" for="inputPassword">新しいパスワード</label>
  <input type="password" id="inputPassword" class="form-control mx-2" required />
  <button type="submit" id="changePassword" class="btn btn-dark" onclick="validateForm();">変化する</button>
...
<p class="w-100 text-center">
  <a data-step="3" data-intro="手掛かりが尽きた場合はここをクリックしてください" id="resign" class="btn btn-dark btn-lg"><i class="fad fa-flag"></i> 辞任</a>
</p>
...
function validateForm() {
  document.getElementById('status').innerHTML = "処理...";
  formData = {
    'ma-phong': '{{ $roomCode }}',
    'pass': $('#inputPassword').val()
  };
  $.ajax({
    url: "{{ url('/api') }}/changePassJa",
    type: "POST",
    data : formData,
    dataType: 'json',
