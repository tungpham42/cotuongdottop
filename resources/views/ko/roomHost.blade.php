...
@section('aboveBoard')
<h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="빨간색 재생">당신은 호스트입니다</h5>
...
<div id="copy-url-black" class="input-group my-1 w-50 mx-auto pulse-light" data-toggle="tooltip" data-placement="bottom" data-original-title="복사하려면 클릭">
  <div class="input-group-prepend">
    <span class="input-group-text" id="url-addon-black"><i class="fal fa-copy"></i></span>
  </div>
  <input data-step="1" data-intro="여기를 클릭하여 링크를 복사하고 친구를 초대하여 함께 플레이하세요" type="text" class="form-control" id="url-black" value="{{ url('/') }}/bang/{{ $roomCode }}/bangmun">
</div>
<p id="room-code" class="w-100 text-center mt-0 mb-1">
  <span data-step="4" data-intro="이 방 코드를 사용하여 매치를 검색하세요" class="alert alert-dark d-inline-block" role="alert" data-toggle="tooltip" data-placement="bottom" data-original-title="이 룸 코드를 복사"><i class="fad fa-trophy-alt"></i> 객실코드: <strong style="cursor: pointer;">{{ $roomCode }}</strong></span>
...
<div data-step="2" data-intro="방의 비밀번호를 변경하려면 여기를 클릭하세요" id="change-pass" class="input-group my-1 w-50 mx-auto">
  <label class="m-auto" for="inputPassword">새 암호</label>
  <input type="password" id="inputPassword" class="form-control mx-2" required />
  <button type="submit" id="changePassword" class="btn btn-dark" onclick="validateForm();">바꾸다</button>
...
<p class="w-100 text-center">
  <a data-step="3" data-intro="단서가 부족한 경우 여기를 클릭하세요" id="resign" class="btn btn-dark btn-lg"><i class="fad fa-flag"></i> 사직하다</a>
</p>
<script>
@if ($room['pass'] != null)
