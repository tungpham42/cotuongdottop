...
@else
<h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="提高您的国际象棋拼图技能">你正在设置拼图</h5>
@endif
<p class="w-100 text-center mt-0 mb-1">
  <a data-step="1" data-intro="设置完成后点击此处下载拼图" id="capture" class="btn btn-danger btn-lg text-light pulse-red" href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="立即另存为图像"><i class="fad fa-download"></i> 下载电路板</a>
  @include('common.tourBtn')
</p>
@endsection
@section('belowContent')
<p class="w-100 text-center mt-0 mb-1">
  <a data-step="2" data-intro="点击这里以解决拼图" id="solve-puzzle" class="btn btn-danger btn-lg pulse-red" href="{{ url('/jiejuenanti') }}"><i class="fad fa-abacus"></i> 解决难题</a>
  @include('common.volumeBtn')
</p>
<p class="w-100 text-center mt-0 mb-1">
  <a data-step="3" data-intro="点击这里保存拼图板" id="new-board" class="w-25 btn btn-dark btn-lg"><i class="fad fa-save"></i> 救生板</a>
  <a data-step="4" data-intro="如果您想回到上一步，请点击这里" id="undo" class="w-25 btn btn-dark btn-lg"><i class="fad fa-undo"></i> 打开</a>
</p>
...
<div id="copy-url" class="input-group my-1 w-50 mx-auto pulse-light" data-toggle="tooltip" data-placement="bottom" data-original-title="点击复制">
  ...
  <input data-step="6" data-intro="请点击这里复制链接并邀请朋友一起玩" type="text" class="form-control" id="url" value="{{ url('/') }}/mi/{{ $board }}">
</div>
<script>
...
$('#new-board').on('click auxclick', function(e){
  if (!game.validate_fen(board.fen() + ' r - - 0 1').valid) {
    ...
    if (removeTrailingSlash("{{ URL::to('/mi/') }}/" + board.fen()) !== removeTrailingSlash(window.location.href)) {
      e.preventDefault();
      $('#AdSenseModal').attr('data-url', "{{ URL::to('/mi/') }}/" + board.fen()).modal('show');
    } else {
      window.location.href = "{{ URL::to('/mi/') }}/" + board.fen();
    }
  }
});
$('#undo').on('click', undo);
