...
@endphp
<span style="background-color: #312e2b; margin-top: -15px;" class="d-block w-100 pb-5 mb-5" id="puzzles"></span>
<div style="background-color: #312e2b" class="container-fluid puzzles px-0">
    <div class="container mx-auto px-3 pt-0">
        <div class="row my-0">
            <h2 class="d-block w-100 text-light ml-3 mb-4"><i class="fas fa-puzzle-piece"></i> {{ numberToWordsEn($puzzles->total()) }} amazing puzzles</h2>
            {{ $puzzles->links('vendor.pagination.en') }}
            @foreach($puzzles as $puzzle)
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4">
                <div class="card shadow-lg rounded border-dark">
                    @if ($puzzle->post_excerpt)
                        <img onclick="setupPuzzle('{{ $puzzle->post_excerpt }}')" class="card-img-top" style="cursor: pointer;" src="{{ $puzzle->getImageAttribute() }}" alt="{{ $puzzle->post_title }}" data-toggle="tooltip" data-placement="top" title='Play this puzzle'>
                    @else
