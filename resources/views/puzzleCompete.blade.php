...
@section('aboveBoard')
<h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="Bàn cờ thế">Bạn đang xếp<span id="puzzle-title"> để thi đấu</span></h5>
@endsection
@section('rightSide')
...
<p class="w-100 text-center mt-0 mb-1">
  <a data-step="1" data-intro="Ấn vào đây để vào trang giải thế cờ" id="solve-puzzle" class="btn btn-danger btn-lg" href="{{ url('/giai-co-the') }}"><i class="fad fa-abacus"></i> Giải cờ thế</a>
  <a data-step="2" data-intro="Ấn vào đây để đặt tên cho thế cờ" id="name-puzzle" class="btn btn-lg btn-dark" href="javascript:void(0);"><i class="fad fa-save"></i> Đặt tên thế cờ</a>
</p>
@if ($board != '')
<p class="w-100 text-center mt-0 mb-1">
...
<div id="copy-url" class="input-group my-1 w-50 mx-auto" data-toggle="tooltip" data-placement="bottom" data-original-title="Ấn để sao chép">
  ...
  <input data-step="3" data-intro="Ấn vào đây để mời bạn bè cùng chơi" type="text" class="form-control" id="url" value="{{ url('/') }}/thi-dau/{{ $board }}">
</div>
<script>
      ...
      if (puzzleName != null) {
        if (puzzleName.trim().length === 0 || puzzleName.length === 0) {
          ...
          $.ajax({
            ...
            data : {
              'name': puzzleName,
              'slug': getSlug(puzzleName, { lang: 'vn' }),
              'fen': board.fen()
            },
            dataType: 'json'
          }).done(function(data){
            if (data.code == '1') {
              $.ajax({
                ...
                // $('#AdSenseModal').attr('data-url', '{{ url('/the-co/') }}' + '/' + getSlug(puzzleName, { lang: 'vn' })).modal('show');
                // $('#adModalCloseBtn').attr('data-original-title', $('#AdSenseModal').attr('data-url'));
                // $('#adModalCloseBtn').tooltip();
                window.location.href = '{{ url('/the-co/') }}' + '/' + getSlug(puzzleName, { lang: 'vn' });
              });
            } else {
              bootbox.alert({
  ...
  $.ajax({
    ...
    if (data.code == '1') {
      $.ajax({
        ...
        window.location.href = '{{ url('/the-co/') }}' + '/' + $('#puzzle-name').val();
      });
    }
    bootbox.alert({
      message: data.message,
